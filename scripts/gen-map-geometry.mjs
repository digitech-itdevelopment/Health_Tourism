...
// The six partner-hospital cities, in flight order down the country. `side` puts the
// label left or right of the dot: at true scale Mumbai and Pune land 8px apart, so
// they only stay legible on opposite sides. `dy` nudges that pair further apart.
const CITIES = {
  india: [
...
/** Great circle, Sydney to Delhi. */
export const ROUTE_PATH = '${greatCircle(SYDNEY, DELHI)}'

/** Great-circle distance Sydney-Delhi, in km, rounded to the nearest 10. */
export const ROUTE_KM = ${Math.round((geoDistance(SYDNEY, DELHI) * 6371) / 10) * 10}
